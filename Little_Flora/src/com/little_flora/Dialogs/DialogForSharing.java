...
public class DialogForSharing extends Dialog implements
android.view.View.OnClickListener{
	...
	public DialogForSharing(Context context,String productName , String productDescription,String fromWhere) {
		super(context);
		// TODO Auto-generated constructor stub
		if ( fromWhere.equals("Sharing_PD"))
			this.context_pd = (ProductDescription) context ;
		else if ( fromWhere.equals("Sharing_PL"))
			this.context_pl = (ProductList_Grid) context ;
		else if ( fromWhere.equals("Sharing_SR"))
	...
	public void onClick(View v) {
		...
		case R.id.txt_sharing_sharewith_facebook:
			context_pd.newFaceBookImplementation() ;
			this.dismiss();
			break;
	...
	public void initView()
	{
		...
		TypeFace_TextView.setTypeFace(getContext(), txt_sharing_sharewith);
		
		txt_sharing_sharewith_facebook = ( TextView ) findViewById(R.id.txt_sharing_sharewith_facebook);
		TypeFace_TextView.setTypeFace(getContext(), txt_sharing_sharewith_facebook);
		...
		TypeFace_TextView.setTypeFace(getContext(), txt_sharing_sharewith_twitter);
		...
		TypeFace_TextView.setTypeFace(getContext(), txt_sharing_sharewith_instagram);
		...
		TypeFace_TextView.setTypeFace(getContext(), txt_sharing_sharewith_pintrest);
	...
	public void sendImageWithSharingMethod(String searchText) {
		...
		try {
			...
			if (!resInfo.isEmpty()) 
			{
				...
				if (!found) {
					if ( fromWhere.equals("Sharing_PD"))
						Toast.makeText(context_pd, searchText+" is not present in your system", Toast.LENGTH_SHORT).show();
					else if ( fromWhere.equals("Sharing_PL"))
						Toast.makeText(context_pl, searchText+" is not present in your system", Toast.LENGTH_SHORT).show();

					return;
				}
				if ( fromWhere.equals("Sharing_PD"))
					context_pd.startActivity(Intent.createChooser(sharingIntent,
						"Select"));
				
				else if ( fromWhere.equals("Sharing_PL"))
					context_pl.startActivity(Intent.createChooser(sharingIntent,
							"Select"));
				else if ( fromWhere.equals("Sharing_SR"))
					context_SR.startActivity(Intent.createChooser(sharingIntent,
							"Select"));
			}

		} catch (Exception e) {
			if ( fromWhere.equals("Sharing_PD"))
				Toast.makeText(context_pd, e.getMessage(), Toast.LENGTH_SHORT).show();
			else if ( fromWhere.equals("Sharing_PL"))
				Toast.makeText(context_pl, e.getMessage(), Toast.LENGTH_SHORT).show();
			else if ( fromWhere.equals("Sharing_SR"))
				Toast.makeText(context_SR, e.getMessage(), Toast.LENGTH_SHORT).show();
		}
	}
